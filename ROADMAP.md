...
- Seed-equivalence (title/audio) follow-up (own branch/PR) — the Day-7 eval reproduced a seed recommending a *different master of itself* at #1 (Take Five → "Take Five — Dave Brubeck", audio cosine 0.988, but a distinct MBID + Deezer track, so the identity dedup above can't catch it). Scoped fix: suppress a result with raw audio cosine ≥ ~0.98 + a close title match to the seed (thread the seed title into `_build_results`); validate it never drops a legitimate live/acoustic version (those score lower audio similarity, so they're safe).
